...
Wagtail DaisyUI Interface Editor is a Django/Wagtail package. The source code is in `src/wagtail_daisIE/`, with tests in `tests/` and a demo Wagtail site in `demo/`.
...
- Tests: pytest with pytest-django. Test settings are in `src/wagtail_daisIE/test/settings.py`.
...
```
src/wagtail_daisIE/       # Package source code
src/wagtail_daisIE/test/  # Django test app with settings, URLs
